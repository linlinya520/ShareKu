...
class PreferencesManager(private val context: Context) {
    ...
    val autoCleanIntervalMinutes: Flow<Int> = context.dataStore.data.map { it[KEY_AUTO_CLEAN_INTERVAL] ?: 0 }
    val lastCleanupTime: Flow<Long> = context.dataStore.data.map { it[KEY_LAST_CLEANUP_TIME] ?: 0L }
    ...
    suspend fun setAutoCleanInterval(minutes: Int) { context.dataStore.edit { it[KEY_AUTO_CLEAN_INTERVAL] = minutes } }
    suspend fun setLastCleanupTime(time: Long) { context.dataStore.edit { it[KEY_LAST_CLEANUP_TIME] = time } }
    ...
    companion object {
        ...
        private val KEY_AUTO_CLEAN_INTERVAL = intPreferencesKey("auto_clean_interval")
